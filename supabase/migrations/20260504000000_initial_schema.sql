-- HabitForge Complete Schema + Seed Data
...
CREATE INDEX IF NOT EXISTS idx_habits_preloaded ON habits(is_preloaded);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='habit_logs' AND policyname='Users can manage own habit logs') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='rewards' AND policyname='Users can manage own rewards') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='reward_logs' AND policyname='Users can manage own reward logs') THEN
    CREATE POLICY "Users can manage own reward logs" ON reward_logs FOR ALL USING (user_id = auth.uid());
  END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_reward_logs_user ON reward_logs(user_id, earned_at);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='punishments' AND policyname='Users can manage own punishments') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='punishment_logs' AND policyname='Users can manage own punishment logs') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_challenges' AND policyname='Users can manage own user_challenges') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='diary_entries' AND policyname='Users can only access own diary entries') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='diary_english_progress' AND policyname='Users can manage own English progress') THEN
...
('21-Day Daily Workout', 'Any workout - minimum 20 minutes. No rest days for 21 days.', 21, '["Minimum 20 minutes of any workout", "No rest days - consistency is the goal", "Log your workout type daily"]', false, true, 0),
('30-Day Cold Shower', 'End every shower with 2 minutes of cold water only.', 30, '["Minimum 2 minutes cold water", "Every single day no exceptions", "Warm shower allowed before cold part"]', true, true, 0),
('14-Day Digital Detox Morning', 'No phone for first 60 minutes after waking.', 14, '["Zero phone for first 60 min", "Alarm clock allowed", "Journal or read instead"]', false, true, 0),
('30-Day Read Daily', 'Read minimum 20 pages every day for 30 days.', 30, '["20 pages minimum per day", "Any book counts - fiction or non-fiction", "Audiobooks do NOT count"]', true, true, 0),
('7-Day No Sugar', 'Zero added sugar for 7 days. Fruits allowed. Restart on fail.', 7, '["Zero added sugar", "Fresh whole fruits allowed", "Restart from Day 1 if you slip"]', true, true, 0)
ON CONFLICT DO NOTHING;
